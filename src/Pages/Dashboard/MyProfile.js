...
const MyProfile = () => {
        ...
        fetch(url, {
            ...
            .then(result => {
                ...
                if (result.success) {
                    ...
                    fetch(`https://plex-tool-server.onrender.com/updateduser?email=${user.email}`, {
                        method: "PUT",
                        headers: {
    ...
    const { data: displayUser, isLoading } = useQuery('user', () => fetch(`https://plex-tool-server.onrender.com/user?email=${user.email}`, {
        method: "GET",
        headers: {
            "authorization": `Bearer ${localStorage.getItem('accessToken')}`,
            "content-type": "application/json"
